...
/**
 * Implements default worker file path.
 */
function defaultWorkerFilePath() {
  return process.env.WORKER_LOG_FILE_PATH?.trim() || '/var/log/homelab/worker/worker.log';
}

/**
 * Builds worker runtime logger.
 */
function buildWorkerRuntimeLogger() {
  const level = parseLogLevel(process.env.WORKER_LOG_LEVEL ?? process.env.LOG_LEVEL, 'info');
  ...
  return new RuntimeLogger({
    serviceName: 'worker',
    level,
    filePath: defaultWorkerFilePath(),
    logToStdout,
    logToFile,
...
/**
 * Implements worker runtime logger.
 */
export const workerRuntimeLogger = buildWorkerRuntimeLogger();
